...
i took this class from thesillydoggo (thx doggo)

*/
class ownDialogObject : public cocos2d::CCObject {
protected:
...
public:
	bool init(std::string title, std::string text, int type, float unknown, bool also_unknown, _ccColor3B textColor) {
		...
		return reinterpret_cast<bool(__thiscall*)(ownDialogObject*, std::string, std::string, int, float, bool, _ccColor3B)>
	...
	static ownDialogObject* create(std::string title, std::string text, int type, float text_scale, bool is_unskippable, _ccColor3B textColor) {
		ownDialogObject* obj = new ownDialogObject();
